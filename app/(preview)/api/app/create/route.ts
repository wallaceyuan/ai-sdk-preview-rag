...
export async function POST(req: Request) {
  ...
  try {
    ...
    const silence = await Kitten.create({ name: 'Silence' });
...
async function handler(req: Request) {
  const { parentId, name, avatar, type, modules, edges, teamId, tmbId } = await req.json();
...
export const onCreateApp = async ({
  ...
  const create = async (session: ClientSession) => {
    ...
    // const silence = await Kitten.create({ name: 'Silence444' });
