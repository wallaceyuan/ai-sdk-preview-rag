...
async function handler(req: NextRequest): Promise<AppListItemType[]> {
  const { parentId, type, getRecentlyChat, searchKey } = await req.json();
  ...
  console.log("    app: ParentApp,tmbId,teamId, tmbPer", ParentApp, tmbId, teamId,tmbPer);
